        ...
        if request.method == "POST":
            ...
            if result is None:
                ...
                if query.cust_id is None:
                    ...
                    temp = CustomerLog(
                        cust_id=query.cust_id, log_message="Customer Created"
                    )
                    db.add(temp)
                    db.commit()
                    flash(
                        f"Customer {query.name} is created with customer ID : {query.cust_id}.",
                        "success",
                    )
...
def viewcustomer(cust_id=None):
    ...
    if session["usert"] == "executive":
        if request.method == "POST":
            ...
            sql_query = text(
                "SELECT * from customers WHERE cust_id = :c or cust_ssn_id = :d"
            )
            data = db.execute(sql_query, {"c": cust_id, "d": cust_ssn_id}).fetchone()
            if data is not None:
                return render_template(
                    "viewcustomer.html", viewcustomer=True, data=data
                )
    ...
    if session["usert"] == "executive":
        if cust_id is not None:
            if request.method != "POST":
                ...
                age = int(request.form.get("age"))
                sql_query = text(
                    "SELECT * from customers WHERE cust_id = :c and status = 'activate'"
                )
                result = db.execute(sql_query, {"c": cust_id}).fetchone()
                if result is not None:
                    sql_query = text(
                        "UPDATE customers SET name = :n , address = :add , age = :ag WHERE cust_id = :a"
                    )
                    result = db.execute(
                        sql_query, {"n": name, "add": address, "ag": age, "a": cust_id}
                    )
                    db.commit()
                    temp = CustomerLog(
                        cust_id=cust_id, log_message="Customer Data Updated"
                    )
                    db.add(temp)
                    db.commit()
    ...
    if session["usert"] == "executive":
        if cust_id is not None:
            ...
            if result is not None:
                ...
                flash(
                    f"Customer with id : {cust_id} is already deactivated or not present in database.",
                    "warning",
                )
    return redirect(url_for("viewcustomer"))
...
def activatecustomer(cust_id=None):
    ...
    if session["usert"] == "executive":
        if cust_id is not None:
            cust_id = int(cust_id)
            sql_query = text(
                "SELECT * from customers WHERE cust_id = :a and status = 'deactivate'"
            )
            result = db.execute(sql_query, {"a": cust_id}).fetchone()
            if result is not None:
                sql_query = text(
                    "UPDATE customers SET status='activate' WHERE cust_id = :a"
                )
                query = db.execute(sql_query, {"a": cust_id})
                db.commit()
                ...
                return redirect(url_for("dashboard"))
            flash(
                f"Customer with id : {cust_id} is already activated or not present in database.",
                "warning",
            )
    return redirect(url_for("viewcustomer"))
...
def customerstatus():
    ...
    if session["usert"] == "executive":
        ...
        if data:
            return render_template(
                "customerstatus.html", customerstatus=True, data=data
            )
        else:
...
def addaccount():
    ...
    if session["usert"] == "executive":
        if request.method == "POST":
            ...
            if result is not None:
                sql_query = text(
                    "SELECT * from accounts WHERE cust_id = :c and acc_type = :at"
                )
                result = db.execute(
                    sql_query, {"c": cust_id, "at": acc_type}
                ).fetchone()
                if result is None:
                    ...
                    if query.acc_id is None:
                        ...
                        flash(
                            f"{query.acc_type} account is created with customer ID : {query.acc_id}.",
                            "success",
                        )
                        return redirect(url_for("dashboard"))
                else:
                    flash(
                        f"Customer with id : {cust_id} has already {acc_type} account.",
                        "warning",
                    )
            else:
                flash(
                    f"Customer with id : {cust_id} is not present in database.",
                    "warning",
                )
    ...
    if session["usert"] == "executive":
        if request.method == "POST":
            ...
            if result is not None:
                # delete from accounts WHERE acc_id = :a and acc_type=:at", {"a": acc_id,"at":acc_type}
                sql_query = text(
                    "UPDATE accounts SET status='deactive' WHERE acc_id = :a and acc_type=:at"
                )
...
def viewaccount():
    if "user" not in session:
        return redirect(url_for("login"))
    if (
        session["usert"] == "executive"
        or session["usert"] == "teller"
        or session["usert"] == "cashier"
    ):
        if request.method == "POST":
...
def viewaccountstatus():
    ...
    if session["usert"] == "executive":
        ...
        if data:
            return render_template(
                "viewaccountstatus.html", viewaccount=True, data=data
            )
        else:
        ...
        if acc_id is None:
            ...
            if request.method == "POST":
                ...
                if data is not None:
                    ...
                    db.commit()
                    flash(
                        f"{amount} Amount deposited into account: {data.acc_id} successfully.",
                        "success",
                    )
                    temp = Transactions(
                        acc_id=data.acc_id,
            ...
            if request.method == "POST":
                ...
                if data is not None:
                    if int(data.balance) >= int(amount):
                        ...
                        db.commit()
                        flash(
                            f"{amount} Amount withdrawn from account: {data.acc_id} successfully.",
                            "success",
                        )
                        temp = Transactions(
                            acc_id=data.acc_id,
                            trans_message="Amount Withdrawn",
                            amount=amount,
                        )
                        db.add(temp)
                        db.commit()
        ...
        if cust_id is None:
            ...
            if request.method == "POST":
                ...
                if src_type != trg_type:
                    sql_query = text(
                        "select * from accounts where cust_id = :a and acc_type = :t and status='active'"
                    )
                    src_data = db.execute(
                        sql_query, {"a": cust_id, "t": src_type}
                    ).fetchone()
                    sql_query = text(
                        "select * from accounts where cust_id = :a and acc_type = :t and status='active'"
                    )
                    trg_data = db.execute(
                        sql_query, {"a": cust_id, "t": trg_type}
                    ).fetchone()
                    if src_data is not None and trg_data is not None:
                        if src_data.balance > amount:
                            ...
                            sql_query = text(
                                "update accounts set balance = :b where cust_id = :a and acc_type = :t"
                            )
                            test = db.execute(
                                sql_query,
                                {"b": src_balance, "a": cust_id, "t": src_type},
                            )
                            db.commit()
                            temp = Transactions(
                            ...
                            db.execute(
                                sql_query,
                                {"b": trg_balance, "a": cust_id, "t": trg_type},
                            )
                            db.commit()
                            temp = Transactions(
                ...
                if data and len(data) == 2:
                    return render_template(
                        "transfer.html", deposit=True, cust_id=cust_id
                    )
                else:
    ...
    if session["usert"] == "teller" or session["usert"] == "cashier":
        if request.method == "POST":
            ...
            if flag == "red":
                ...
                data = db.execute(
                    sql_query, {"a": acc_id, "s": start_date, "e": end_date}
                ).fetchall()
            if data:
                return render_template(
                    "statement.html", statement=True, data=data, acc_id=acc_id
                )
            else:
...
def pdf_xl_statement(acc_id=None, ftype=None):
    ...
    if session["usert"] == "teller" or session["usert"] == "cashier":
        if acc_id is not None:
            sql_query = text(
                "SELECT * FROM transactions WHERE acc_id=:a order by time_stamp limit 20;"
            )
    ...
    if request.method == "POST":
        ...
        if result is not None:
            if bcrypt.check_password_hash(result.password, passw) is True:
                ...
                flash(
                    f"{result.name.capitalize()}, you are successfully logged in!",
                    "success",
                )
                return redirect(url_for("dashboard"))
...
def customerlog():
    ...
    if session["usert"] == "executive":
        if request.method == "POST":
            ...
            data = db.execute(sql_query, {"c": cust_id}).fetchone()
            t = {"message": data.log_message, "date": data.time_stamp}
            return jsonify(t)
        else:
            dict_data = []
            sql_query = text(
